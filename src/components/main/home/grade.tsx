                ...
                <div className="d-flex flex-column justify-content-center">
                    <div className="d-flex gap-4 py-4 px-3">
                        ...
                        <div className="d-flex flex-column justify-content-center gap-1">
                            <p className="m-0 fs-5"><strong>{item.title}</strong></p>
                            <p className="m-0 text-muted d-inline">{item.location}<span> - {item.time}</span></p>
                        </div>
                    </div>
                    <hr className="m-0" />
                </div>
...
const Item = (props: GradeItemProps) => {
    const { time, title, location } = props;

    return(
        <div className="p-3">
            <div>
                <img src="#" alt="relógio" />
                <p className="grade-time">{time}</p>
            </div>
            <div>
                <p className="grade-description">{title}</p>
                <p className="grade-time">{location}</p>
            </div>
        </div>
    )
}
